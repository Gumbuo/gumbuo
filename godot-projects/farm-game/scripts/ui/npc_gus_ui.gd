...
const BORDER_COLOR := Color(0.55, 0.45, 0.15)
const TITLE_COLOR  := Color(0.90, 0.78, 0.35)
...
func _build_ui() -> void:
	...
	desc.text     = "Take a chicken coop and a beehive, get your farm's little critters going. One-time offer."
...
func _on_claim_pressed() -> void:
	if ResourceManager.claim_coop_and_beehive():
		_status_lbl.text = "Claimed! Check your backpack."
	_refresh()

func _refresh() -> void:
	if ResourceManager.has_claimed_coop_and_beehive():
		_claim_btn.text = "Already Claimed"
		_claim_btn.disabled = true
	else:
		_claim_btn.text = "Claim Coop & Beehive"
