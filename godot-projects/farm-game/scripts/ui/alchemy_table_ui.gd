...
const STATION_TITLE  := "ALCHEMY TABLE"
const BORDER_COLOR   := Color(0.42, 0.18, 0.65)
const TITLE_COLOR    := Color(0.85, 0.60, 1.00)
const ACTION_LABEL   := "BREW"

const RECIPES: Dictionary = {
	"purple_potion":          {"desc": "Enables x10 bulk crafting at all stations.",                 "price_gold": 0.05},
	"red_potion":             {"desc": "Enables x100 bulk crafting at all stations.",                "price_gold": 1.0},
	"combat_health_potion":   {"desc": "This potion help recover 15 health points during fights.",   "ing": {"tomato": 30, "orange_bluegill": 3, "blue_bluegill": 3, "yellow_bluegill": 3}, "energy": 1},
	"combat_stamina_potion":  {"desc": "This potion help recover 15 stamina points during fights.",  "ing": {"blue_flower": 30, "crucian_carp": 6},                                        "energy": 1},
}

const RES_NAME: Dictionary = {
	"tomato":         "Tomato",
	"orange_bluegill":"Orange Bluegill",
	"blue_bluegill":  "Blue Bluegill",
	"yellow_bluegill":"Yellow Bluegill",
	"blue_flower":    "Blue Flower",
	"crucian_carp":   "Crucian Carp",
}
const RES_COLOR: Dictionary = {
	"tomato":         Color(0.90, 0.20, 0.10),
	"orange_bluegill":Color(0.90, 0.50, 0.10),
	"blue_bluegill":  Color(0.30, 0.55, 0.90),
	"yellow_bluegill":Color(0.95, 0.85, 0.10),
	"blue_flower":    Color(0.30, 0.55, 0.92),
	"crucian_carp":   Color(0.60, 0.72, 0.50),
}
const ITEM_COLOR: Dictionary = {
	"purple_potion":         Color(0.55, 0.10, 0.75),
	"red_potion":            Color(0.85, 0.15, 0.15),
	"combat_health_potion":  Color(0.85, 0.15, 0.15),
	"combat_stamina_potion": Color(0.20, 0.35, 0.90),
}
...
func _make_card(parent: Control, recipe_id: String, data: Dictionary) -> Dictionary:
	...
	brew_btn.position = Vector2(730, 6)
	brew_btn.size     = Vector2(88, 26)
	...
	if is_shop:
		return {"craft": brew_btn, "is_shop": true, "price_gold": data.get("price_gold", 0.0)}

	var brew10_btn := Button.new()
	brew10_btn.position = Vector2(730, 36)
	brew10_btn.size     = Vector2(88, 26)
	brew10_btn.text     = "x10"
	...
	brew100_btn.position = Vector2(730, 66)
	brew100_btn.size     = Vector2(88, 26)
...
func _try_craft(recipe_id: String, data: Dictionary, count: int) -> void:
	if data.has("price_gold"):
		var cost: float = data["price_gold"] * count
		if not PlayerData.spend_gold(cost): return
...
func _refresh_states() -> void:
	...
	for card_info in _cards:
		if card_info.get("is_shop", false):
			var cb: Button = card_info["craft"]
			if is_instance_valid(cb):
				cb.disabled = PlayerData.gold < card_info.get("price_gold", 0.0)
			continue
