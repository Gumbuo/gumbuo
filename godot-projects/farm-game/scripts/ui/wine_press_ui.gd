...
const STATION_TITLE_OWN     := "WINE PRESS"
const STATION_TITLE_VISITOR := "WINE PRESS  (visitor — 30 grapes / press)"
const BORDER_COLOR   := Color(0.42, 0.08, 0.25)
const TITLE_COLOR    := Color(0.92, 0.55, 0.70)
const ACTION_LABEL   := "PRESS"

# Recipes will be filled in here. x10 needs purple_potion, x100 needs red_potion.
const RECIPES: Dictionary = {
	"grape_must": {"desc": "Pressed grape must. Used to make wine in a barrel.", "ing": {"grapes": 15}, "energy": 3},
}

const RES_NAME:  Dictionary = {"grapes": "Grapes"}
const RES_COLOR: Dictionary = {"grapes": Color(0.45, 0.12, 0.55)}
const ITEM_COLOR:Dictionary = {"grape_must": Color(0.55, 0.18, 0.40)}

var _inv_lbl:        Label  = null
var _cards:          Array  = []
var _is_visitor:     bool   = false
var _owner_tile_id:  String = ""
...
func _build_ui() -> void:
	...
	title.text     = STATION_TITLE_VISITOR if _is_visitor else STATION_TITLE_OWN
...
func _try_craft(recipe_id: String, data: Dictionary, count: int) -> void:
	var ing: Dictionary = data.get("ing", {})
	var cost_mult: int  = 2 if _is_visitor else 1
	var scaled_ing := {}
	for res_id in ing:
		scaled_ing[res_id] = ing[res_id] * cost_mult
	if not _can_afford(scaled_ing, count): return
	...
	for res_id in scaled_ing:
		ResourceManager.remove_item(res_id, scaled_ing[res_id] * count)
	ResourceManager.add_item(recipe_id, count)
	if _is_visitor and _owner_tile_id != "":
		LandManager.add_to_passive_vault(_owner_tile_id, recipe_id, count)
...
func _can_afford(ing: Dictionary, count: int) -> bool:
	var cost_mult: int = 2 if _is_visitor else 1
	for res_id in ing:
		if ResourceManager.get_count(res_id) < ing[res_id] * cost_mult * count:
			return false
	return true
...
func _refresh_states() -> void:
	...
	for card_info in _cards:
		...
		for p in pills:
			...
			var cost: int  = p["req"] * (2 if _is_visitor else 1)
			lbl.text     = "%s %d/%d" % [RES_NAME.get(p["res_id"], p["res_id"]), have, cost]
			lbl.modulate = Color(0.3, 0.7, 1.0) if have >= cost else Color(1.0, 0.4, 0.4)
