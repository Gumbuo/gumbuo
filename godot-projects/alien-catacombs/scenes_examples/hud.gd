...
var menu_packed:=preload("res://scenes_examples/menu.tscn")
...
func _on_button_inventory_toggled(button_pressed):
	if !$animation_preview.is_playing():
		...
		button_inventory.set_pressed_no_signal(!button_pressed)
...
func _on_button_menu_toggled(button_pressed):
	if not $anim.is_playing():
		...
		button_menu.set_pressed_no_signal(!button_pressed)
